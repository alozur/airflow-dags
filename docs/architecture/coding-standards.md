...
Ruff is configured entirely inside `[tool.ruff]` in `pyproject.toml` — there is
no `.ruff.toml`/`ruff.toml` file. The gate is blocking: see
[`.github/workflows/lint.yml`](../../.github/workflows/lint.yml) for the CI
job (issue #269).

- `line-length = 120` — measured on the repo at ruff-enforcement time: 88
  produced 2,993 `E501` violations, 120 produced 80, 140 produced 22. 120 is
  the practical floor that doesn't require a mass rewrap.
- `target-version` is deliberately **absent**. Ruff infers it from
  `requires-python = ">=3.12"` in `[project]`. Do not add it back — a
  hardcoded `target-version` drifting out of sync with `requires-python` is
  exactly what caused this doc to previously say `py311`.
- `select = ["E", "W", "F", "I", "N", "UP", "B", "C4", "C90", "SIM"]` —
  pycodestyle, pyflakes, isort, pep8-naming, pyupgrade, flake8-bugbear,
  flake8-comprehensions, **mccabe complexity (`C90`)**, and flake8-simplify.
...
1. **Inline `# noqa: CODE - reason`** — for a single, deliberate, documented
   exception (house style; see `utils/llm_cache.py`). Use this when the
   violation is intentional and unlikely to ever need fixing.
2. **`[tool.ruff.lint.per-file-ignores]` baseline** — for debt to be burned
   down later, never for a deliberate exception. The block is generated by
   `scripts/gen_ruff_baseline.py` from `ruff check . --output-format json`
   and lives between the `# --- ruff baseline: generated, do not edit by
   hand ---` / `# --- end ruff baseline ---` sentinels in `pyproject.toml`.
   It is **remove-only**: entries may be deleted (debt paid) or shrink as
   codes are fixed, but nothing may be added to turn CI green — fix the
   code or use an inline `noqa` instead. The `C901` entries in that block
   *are* the tracked complexity backlog (see issue #272); that issue prunes
   the existing list, it does not re-derive a different one.

To verify the baseline has no drift against the current tree (a local
review ritual, not a blocking CI job):
