...
(POSTGRES_USER/POSTGRES_SCHEMA — airflow_dev/development on DEV,
airflow_prod/production on PROD; always a least-privilege DML-only role).
Prints counts and booleans only, never row content or credentials.
"""
...
def main():
    ...
    with pg.get_connection() as conn, conn.cursor() as cur:
        ...
        assert current_user == expected_role, f"Unexpected runtime role: {current_user}"
        ...
        assert table_count > 0, f"Schema {schema} has no tables after migrations"

        # DML round-trip proof, rolled back on purpose: never leaves data behind.
        cur.execute(f"INSERT INTO {schema}.schema_migrations (migration) VALUES (%s)", ("app_smoke_probe",))
        conn.rollback()
