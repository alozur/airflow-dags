...
- [x] 3.1 RED: write `test_verify_final_copy_thumbnail_text_finding_triggers_one_claim_and_trigger` — GIVEN `verdict.findings` includes `field == "thumbnail_text"`, WHEN `_verify_final_copy` (t6b) runs, THEN `claim_thumbnail_text_regeneration` and `_regenerate_flagged_thumbnail` are each called exactly once (spec: "A thumbnail-text finding triggers a regeneration attempt").
- [x] 3.2 RED: write `test_verify_final_copy_no_thumbnail_text_finding_zero_claims` — GIVEN no such finding, THEN zero calls to `claim_thumbnail_text_regeneration` (spec: "No thumbnail-text finding, no regeneration"). Mutation check: temporarily always call claim and confirm this test fails.
- [x] 3.3 GREEN: in `congress_videos/youtube_upload_dag.py::_verify_final_copy` (~line 1420, after the title-reject `raise` at line 1414-1419), add the branch: on a `thumbnail_text` finding, call `db.claim_thumbnail_text_regeneration(output_path, prior_brief=...)`; if claimed, call `_regenerate_flagged_thumbnail(...)`. **Deviation**: `_regenerate_flagged_thumbnail`'s child conf was minimal in PR2 (missing `generic_thumbnail_generator`'s required `youtube_video_id`/`chapter_id`/`debate_summary`/`session`/`domain` keys). Resolved per the orchestrator's explicit direction: the helper's signature gained a `thumbnail_config: dict` first parameter, built the same way `trigger_thumbnail_generation` (t4) builds its own child conf, with `output_path` always overridden to the triggering turn's own file (D6). Extracted into `_build_regen_child_conf` and the claim step into `_claim_and_regenerate_thumbnail` / `_apply_thumbnail_regeneration_if_flagged` to keep both `_regenerate_flagged_thumbnail` and `_verify_final_copy` under the repo's C901 complexity gate (issue #269).
- [x] 3.4 RED: write `test_verify_final_copy_hoisted_xcom_push_fires_without_correction` — GIVEN `verdict.correction_applied == False` AND a landed thumbnail regeneration, WHEN `t6b` completes, THEN `ti.xcom_push(key="upload_config", ...)` IS called (regression guard for the non-negotiable hoist — today's push lives inside `if verdict.correction_applied:` at line 1426 and would silently drop a correction-free regeneration). Mutation check executed live: temporarily reverted the hoist to the pre-#545 shape, confirmed this exact test fails, then restored it.
- [x] 3.5 GREEN: hoist the `ti.xcom_push(key="upload_config", value=config)` at line 1426 out of `if verdict.correction_applied:` into a single guarded push covering both the correction mutation and the thumbnail-file swap (design D5: `mutated = True` pattern, ONE push).
- [x] 3.6 GREEN: on a landed regeneration with an existing output file (`os.path.exists`), set `video["thumbnail_file"] = regen["output_path"]` before the hoisted push (design D5).
- [x] 3.7 RED: write `test_verify_final_copy_sibling_isolation_by_output_path` — GIVEN two sibling turns sharing `chapter_id` but distinct `output_path`/`video_file`, WHEN turn A's regeneration is triggered, THEN the triggered child `conf["output_path"]` equals turn A's own `video["video_file"]`, never the shared `chapter_id` or turn B's path (spec: "A sibling turn is unaffected by another turn's regeneration"; design D6).
- [x] 3.8 RED: write `test_verify_final_copy_every_failure_mode_returns_none_never_raises` — parametrized over all six outcomes (`timeout`, `trigger_failed`, `child_failed`, `invalid_result`, `not_claimed`, DB-claim-exception) asserting `_verify_final_copy` returns `None`, `upload_config` XCom is unmutated when no correction/regen landed, and no exception propagates (spec: "No Code Path May Block Or Indefinitely Delay Publication"). Mutation check executed live: let the claim-time exception re-raise from `_claim_and_regenerate_thumbnail`, confirmed the `claim_exception` parametrized case fails, then reverted.
...
- [x] 3.10 GREEN: confirm 3.3's branch placement (strictly after line 1419's `raise`) satisfies 3.9 without additional code.
- [x] 3.11 GREEN: add one line to `_copy_verification_problems` (~line 703-750) surfacing "a `thumbnail_text` finding was raised and regeneration did not land" as an informational, non-blocking entry feeding `_check_upload_failures` (design D4 operator signal). RED test first: `test_copy_verification_problems_reports_unlanded_thumbnail_regen` (plus a positive counterpart, `test_copy_verification_problems_landed_regen_is_not_a_finding`).
- [x] 3.12 Confirm no Airflow `execution_timeout` is added to the `t6b` `PythonOperator` — add a regression comment near the operator definition citing design D4's rejected-alternative rationale (a task timeout fails `t6b`, skips `t7`, converting a finding into a publication block). Review checkpoint, not a test: verified `git diff`/`grep execution_timeout` shows no such kwarg on the `t6b` `PythonOperator`.
- [x] 3.13 Update `docs/DAGS.md` recording: the 1000s poll bound, the 2-attempt spend ceiling, and the audit-only (non-authoritative) semantics of the `video_thumbnails` row for siblings. **Deviation**: `CONTEXT.md`/`docs/adr/` do not exist in this repo yet and, per `docs/agents/domain.md`, are created lazily by `/domain-modeling` — not upfront by an unrelated feature PR. `docs/DAGS.md` is this repo's actual living DAG-behavior doc (already documents #512's `verify_final_copy`), so the #545 addition landed there instead, in Spanish matching the rest of that file.
- [x] 3.14 Run `uv run pytest tests/congress_videos/test_youtube_upload_dag.py` in full; run `bash scripts/test-airflow-e2e.sh` to confirm `congress_videos/**` DAG import stays clean. Full-repo `uv run pytest` also run (see apply-progress.md for exact counts). `test-airflow-e2e.sh` reported `EXIT_DOCKER_UNAVAILABLE` (exit 4) — Docker is genuinely unavailable in this environment; not a failure.
