...
- [x] Confirm chain strategy with the user/orchestrator before the first PR is opened. Resolved: `stacked-to-dev` (dev is this repo's real integration branch, not main). <!-- sdd-owner: parent -->
- [x] Manual user review of PR1 (uv migration, T1-T3): user reviewed the diff directly and authorized commit. <!-- sdd-owner: parent -->
- [~] Bounded native review for PR1-PR3 (combined candidate): attempted via `gentle_review` wrapper (blocked, `applicability: corrupted`) and then directly via native `gentle-ai review` CLI (lineage `review-74e1128442cf4008` created; all 4 canonical lenses — risk, resilience, readability, reliability — run for real by the parent orchestrator and captured via `gentle-ai review capture-result`, zero BLOCKER/CRITICAL findings, 3 non-blocking WARNING/SUGGESTION findings). `gentle-ai review finalize` failed with `invalid compact authority graph: json: unknown field "evidence_class"` — pre-existing corruption in this repository's `.git/gentle-ai/review-transactions/v2/` store from prior sessions (multiple stale, never-finalized lineages; at least one with an incompatible schema), unrelated to this change's content. `gentle-ai review repair --preflight` reported 0 eligible candidates; `gentle-ai review invalidate` on the stale lineages requested flags/authorization not available to the parent. No approved native receipt was obtainable in this session. <!-- sdd-owner: parent -->
- [x] Commit and push to `dev` (stacked-to-dev): done manually by the user via `git commit`/`git push` directly (both blocked for the parent's own execution by the same pre-commit/pre-push harness gate requiring the native receipt above). 3 commits (`b3a7f81` uv migration, `6f4044f` e2e infra, `120e563` wiring+validation+SDD docs), rebased cleanly onto `origin/dev` (which had advanced 5 commits during this change's implementation), pushed successfully. <!-- sdd-owner: parent -->
