    ...
    def _extract_and_pretrim_clip(ti, **context):
        ...
        for chapter in chapters:
            ...
            if not source_video_path:
                logging.warning("No source video for video_id=%s chapter_id=%s — skipping", video_id, chapter_id)
                continue
            ...
            if not result["success"]:
                logging.warning("Chapter %s extraction failed: %s — skipping", chapter_id, result.get("error"))
                continue
            ...
            if duration < 120:
                logging.warning(
                    "Chapter %s clip duration %.1fs < 120s minimum after trim — skipping", chapter_id, duration
                )
                continue
