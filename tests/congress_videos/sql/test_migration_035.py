...
MIGRATION_PATH = (
    Path(__file__).resolve().parents[3] / "congress_videos" / "sql" / "migrations" / "035_add_min_turn_duration.sql"
)
...
ALL_034_AND_NEW_SELECT_EXPRESSIONS = INHERITED_034_SELECT_EXPRESSIONS + NEW_GROUP_SPAN_SELECT_EXPRESSIONS
...
class TestMigration035BugClassGuards:
    ...
    def test_no_naive_representative_row_duration_predicate(self):
        ...
        sql = MIGRATION_PATH.read_text(encoding="utf-8").upper()
        assert not re.search(r"(ST|DEDUP)\.END_SECONDS\s*-\s*(ST|DEDUP)\.START_SECONDS", sql), (
