...
MIGRATION_PATH = (
    Path(__file__).resolve().parents[3] / "congress_videos" / "sql" / "migrations" / "042_thumbnail_republish.sql"
)
...
class TestMigration042FileExists:
    ...
    def test_filename_sorts_after_041(self):
        ...
        assert names.index(MIGRATION_PATH.name) > names.index("041_analytics_actions.sql")
